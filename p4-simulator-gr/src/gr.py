    ...
    def runBatch(self, OBS_AGENT, quality=None, density=None, distribution=None):
        ...
        with open(self.infile, 'r') as f:
            ...
            for problem in reader:
                ...
                print('agent loaded')
                if OBS_AGENT == "q_agent":
                    self.obs_agent.preprocess(self.model)
    ...
    def outputLine(self, outfile, writearray, goals):
        try:
            if not os.path.isfile(outfile):
                headerlist = ["map", "agent", "start", "optcost", "density", "distribution", "formula"]
                ...
                for counter in range(MAX_GOALS):
                    headerlist.extend(["goal" + str(counter), "costdif", "cost", "probability"])
...
class GoalObj(object):
    ...
    def getData(self):
        ...
        c = '{:8.3f}'.format(self.cost)
        return [(self.coord[0], self.coord[1]), cd, c, pr]
